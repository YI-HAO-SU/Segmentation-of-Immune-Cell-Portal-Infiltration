...
from utils.losses import TverskyLoss, focal_loss
...
def forward_step(model, batch, loss_functions, device):
    ...
    # loss = loss_tversky_region + loss_tversky_border + loss_focal
    loss = loss_tversky_region + loss_tversky_border
    ...
    loss_dict = {
        'total': loss,
        'loss_tversky_region': loss_tversky_region,
        'loss_tversky_border': loss_tversky_border,
        # 'loss_focal': loss_focal
    }

    return loss, loss_dict, log_images, log_iou
...
if __name__ == "__main__":
    ...
    for epoch_idx in range(args.max_epoch):
        ...
        train_loss_sum = []
        train_tversky_region_sum = []
        train_tversky_border_sum = []
        train_focal_sum = []
        ...
        for batch_idx, batch in tqdm(enumerate(train_dataloader), 
            ...
            train_loss, loss_dict, log_images, log_iou = forward_step(model, batch, 
            ...
            train_tversky_region_sum += [loss_dict['loss_tversky_region'].item()]
            train_tversky_border_sum += [loss_dict['loss_tversky_border'].item()]
            # train_focal_sum += [loss_dict['loss_focal'].item()]
            train_iou_sum += [log_iou['iou']]
            train_border_iou_sum += [log_iou['border_iou']]
        ...
        print("train_avg_tversky_region:",  sum(train_tversky_region_sum) / len(train_tversky_region_sum))
        print("train_avg_tversky_border:",  sum(train_tversky_border_sum) / len(train_tversky_border_sum))
        # print("train_avg_focal:", sum(train_focal_sum) / len(train_focal_sum))
        ...
        val_iou_sum = []
        val_border_iou_sum = []
        val_tversky_region_sum = []
        val_tversky_border_sum = []
        val_focal_sum = []

        for batch in tqdm(val_dataloader, total=len(val_dataloader)):
            model.eval()
            with torch.no_grad():
                val_loss, loss_dict, log_images, log_iou = forward_step(model, 
                                                            batch, 
                                                            loss_functions, 
                                                            device)

            val_loss_sum += [val_loss.item()]
            val_tversky_region_sum += [loss_dict['loss_tversky_region'].item()]
            val_tversky_border_sum += [loss_dict['loss_tversky_border'].item()]
            # val_focal_sum += [loss_dict['loss_focal'].item()]
        ...
        print("val_avg_tversky_region:",  sum(val_tversky_region_sum) / len(val_tversky_region_sum))
        print("val_avg_tversky_border:",  sum(val_tversky_border_sum) / len(val_tversky_border_sum))
        # print("val_avg_focal:", sum(val_focal_sum) / len(val_focal_sum))
